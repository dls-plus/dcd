...
ScopeSymbolPair generateAutocompleteTrees(const(Token)[] tokens,
	...
	size_t cursorPosition, ref ModuleCache cache, string path = "stdin")
{

	Module m = parseModuleForAutocomplete(tokens, istring(path),
		parseAllocator, cursorPosition);

	scope first = new FirstPass(m, istring(path), symbolAllocator,
