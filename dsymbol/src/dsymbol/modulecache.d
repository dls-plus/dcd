...
struct ModuleCache
{
	 ...
	 */
	DSymbol* cacheModule(string location, bool force = false)
	{
		...
		if (!force && !needsReparsing(cachedLocation))
		...
		if (oldEntry != null)
		{
			...
			generateUpdatePairs(oldEntry.symbol, first.rootSymbol.acSymbol, updatePairs);

			// Apply updates to all symbols in modules that depend on this one

			// update types from old pair in case someone still hold a reference to it
			
			string[] tocheck;
			foreach(a; cache)
			{
				foreach(dep; a.dependencies)
				{
					if(dep == cachedLocation)
					{
						a.symbol.updateTypes(updatePairs);
						tocheck ~= a.path;
						resolveDeferredTypes(a.path);
					}
				}
			}
			...
			foreach(a; cache)
			{
				if(a.path == cachedLocation) continue;
				foreach(dep; a.dependencies)
				{
					foreach(chk; tocheck)
					{
...
private:
	...
	public CacheEntry* getEntryFor(istring cachedLocation)
	{
		CacheEntry dummy;
