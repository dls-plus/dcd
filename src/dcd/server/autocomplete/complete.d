	...
	ref ModuleCache moduleCache)
{
	...
	// ensure the dependencies of the moduel the cursor is in are up to date
	if (path.length != 0 && path != "stdin")
	{
		import std.string;
		import std.array;
		import dsymbol.string_interning;
		...
		// vscode adds that weird file:// thingy
		path = replace(path, "file:///", "");
		version(Windows)
		{
			// on windows it encodes the drive letter 'x:'
		...
		auto msym = moduleCache.cacheModule(path, true);

		string[] tocache;
		auto cachedLocation = istring(path);
		auto cached = moduleCache.getEntryFor(cachedLocation);

		// if the current module is cached
		if (cached)
		{
			// check its dependencies
			foreach(dep; cached.dependencies[])
			{
				auto cachedDep = moduleCache.getEntryFor(dep);
				if(!cachedDep) continue;

				// dependency found, let's add it so we can reparse it
				tocache ~= dep;
			}
		}
		...
		foreach (s; noDupes(tocache))
		{
			// force reparse it
			auto sym = moduleCache.cacheModule(s);
		}
	}
			...
			|| beforeTokens[$ - 1] == tok!",")
		{
			...
			if (end != size_t.max)
				return parenCompletion(beforeTokens[0 .. end], tokenArray,
					request.cursorPosition, moduleCache, path);
		}
		else
		{
	...
	return dotCompletion(beforeTokens, tokenArray, request.cursorPosition, moduleCache, path);
}
...
AutocompleteResponse dotCompletion(T)(T beforeTokens, const(Token)[] tokenArray,
	size_t cursorPosition, ref ModuleCache moduleCache, string path)
{
	...
	case tok!"]":
		...
		ScopeSymbolPair pair = generateAutocompleteTrees(tokenArray, allocator,
			&rba, cursorPosition, moduleCache, path);
...
AutocompleteResponse parenCompletion(T)(T beforeTokens,
	const(Token)[] tokenArray, size_t cursorPosition, ref ModuleCache moduleCache, string path)
{
	AutocompleteResponse response;
