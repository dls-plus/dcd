	...
	serverLoop: while (true)
	{
		...
		else
		{
			...
			else if (request.kind & RequestKind.doc)
			{
				...
				s.trySendResponse(getDoc(request, cache, "stdin"), "Could not get DDoc information");
			}
			else if (request.kind & RequestKind.symbolLocation)
				s.trySendResponse(findDeclaration(request, cache, "stdin"), "Could not get symbol location");
			else if (request.kind & RequestKind.search)
				s.sendResponse(symbolSearch(request, cache, "stdin"));
			else if (request.kind & RequestKind.localUse)
				s.trySendResponse(findLocalUse(request, cache, "stdin"), "Couldnot find local usage");
		}
